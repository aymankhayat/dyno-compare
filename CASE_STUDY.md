...
- **Tests in the browser.** `tests.html` runs 31 checks on conversions, data integrity, URL state and the trade-off generator.
...
### 3. Cars modeled in code, to scale
- **Problem:** The first 3D pass extruded generic body styles and looked nothing like the real cars. Downloaded 3D models would have raised licensing questions and been several megabytes each.
- **Approach:** Each body design is a hand-traced silhouette, extruded and sculpted in three.js, then sized from the maker's published length, width, height and wheelbase, with signature details (911 round headlights and light bar, M3 kidney grille, GR Corolla wing and triple exhaust, IONIQ 5 N pixel lights).
- **Result:** 6 models in 19 KB of code and 0 model files. One honest gap: Porsche's US site lists no 911 height, so those two models use a third-party figure, noted in `js/data.js`. This only affects the 3D shape, not any displayed spec.
